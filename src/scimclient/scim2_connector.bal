...
public function <ScimConnector scimCon> init (string baseUrl, string accessToken, string clientId, string clientSecret,
    ...
    oauthCon.init(baseUrl, accessToken, clientId, clientSecret, refreshToken, refreshTokenEP, refreshTokenPath, location, password);
    baseURL = baseUrl;
...
public function <ScimConnector scimCon> getListOfUsers () (User[], error) {
    ...
    response, connectorError = oauthCon.get(SCIM_USER_END_POINT, request);
...
@Param {value:"error: Error"}
public function <ScimConnector scimConn> getListOfGroups () (Group[], error) {
    ...
    response, connectorError = oauthCon.get(SCIM_GROUP_END_POINT, request);
    if (connectorError != null) {
        Error = {message:failedMessage + connectorError.message, cause:connectorError.cause};
        return null, Error;
    }
...
@Param {value:"error: Error"}
public function <ScimConnector scimConn> getMe () (User, error) {
...
@Param {value:"error: Error"}
public function <ScimConnector scimConn> getGroupByName (string groupName) (Group, error) {
